...
use App\Modules\Dashboard\Models\Banner;
use Config\App;
...
class Auth extends BaseController
{
	...
	{
		if ($this->ionAuth->loggedIn()) return redirect()->to(base_url("/"));
		...
		if ($this->request->getPost() && $this->validation->withRequest($this->request)->run()) {
			...
			if ($this->ionAuth->login($this->request->getVar('identity'), $this->request->getVar('password'), $remember)) {
				...
				$this->session->setFlashdata('message', 'Login Sukses');
			...
			$this->data['login_image'] = 'login_image.jpg';
	...
	public function delete_user($id)
	{
		...
		$this->session->setFlashdata('message', 'Hapus User Berhasil');
	...
	public function delete_group($group_id)
	{
		...
		$this->session->setFlashdata('message', 'Hapus Group Berhasil');
		$this->ionAuth->deleteGroup($group_id);
		return 	redirect('auth/groups', 'refresh');
	}
	public function groups()
	{
		...
		// echo '</pre>';
		// die;
		return $this->renderPage($this->viewsFolder . DIRECTORY_SEPARATOR . 'groups', $this->data);
	}

	public function noakses()
	{
		$this->session->setFlashdata('message', 'You dont have access!!');
		return redirect()->to('/');
	}
}
